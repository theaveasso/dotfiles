...
mocha_1.suite("Parser Tests", function () {
    const tomlFile = Buffer.from(fs.readFileSync("./src/test/full.toml")).toString();
    // Defines a Mocha unit test
    mocha_1.test("Read File", function () {
        assert.notEqual(tomlFile, undefined);
    });
    mocha_1.test("Read Tables", function () {
        const doc = parser_1.parse(tomlFile);
        const expected = [
            "package",
    ...
    mocha_1.test("Read Values", function () {
        const doc = parser_1.parse(tomlFile);
        const expected = [4, 1, 1, 1, 7, 2, 2, 1, 1, 1, 1, 5, 1, 2, 5, 1];
        assert.equal(doc.values.length, expected.length);
        for (let i = 0; i < expected.length; i++) {
            assert.equal(doc.values[i].values.length, expected[i], `Value count is wrong for table: ${doc.values[i].key}, items: ${JSON.stringify(doc.values[i].values, null, 2)}`);
        }
    });
    mocha_1.test("Test Start & end", function () {
        const doc = parser_1.parse(tomlFile);
        {
    ...
    mocha_1.test("FilterCrates", function () {
        const doc = parser_1.parse(tomlFile);
        const expected = [
            'lazy_static = "1.0.1"',
            'libc = "0.2.42"',
        ...
        const actual = parser_1.filterCrates(doc.values);
        assert.equal(actual.length, expected.length);
        for (let i = 0; i < expected.length; i++) {
